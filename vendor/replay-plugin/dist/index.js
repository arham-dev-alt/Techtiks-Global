...
// src/build-transform.ts
var VIRTUAL_STATIC_GET_PREFIX = "virtual:shibuk-replay/static-get/";
var RESOLVED_VIRTUAL_STATIC_GET_PREFIX = `\x00${VIRTUAL_STATIC_GET_PREFIX}`;
var isAstNode = (value) => {
  return Boolean(value) && typeof value === "object" && typeof value.type === "string" && typeof value.start === "number" && typeof value.end === "number";
};
var walkAst = (root, visit) => {
  if (!isAstNode(root)) {
    return;
  }
  const stack = [root];
  while (stack.length > 0) {
    ...
    if (!isAstNode(current2)) {
      continue;
    }
    visit(current2);
    for (const value of Object.values(current2)) {
...
var toRouteRequestKey = (route) => {
  return `${route.pathname}${route.search}`;
};
var buildVirtualId = (route) => {
...
var isFetchCallee = (callee) => {
  ...
  const object = isAstNode(memberExpression.object) && memberExpression.object.type === "Identifier" ? memberExpression.object.name : null;
  const property = isAstNode(memberExpression.property) && memberExpression.property.type === "Identifier" ? memberExpression.property.name : null;
  return property === "fetch" && (object === "window" || object === "globalThis");
};
var isCompatibleStaticGetInit = (node) => {
  if (!isAstNode(node)) {
    return false;
  }
...
var normalizeStaticGetInitProperty = (property) => {
...
var normalizeAbsoluteFetchLiteral = (value) => {
  try {
...
var resolveRouteForFetchLiteral = (routeIndex, literal2) => {
  const absoluteUrl = normalizeAbsoluteFetchLiteral(literal2);
  if (absoluteUrl) {
    return routeIndex.byAbsoluteUrl.get(absoluteUrl) ?? null;
  }
  const requestKey = normalizeRelativeFetchLiteral(literal2);
  if (!requestKey) {
    return null;
  }
  return routeIndex.byRelativeRequestKey.get(requestKey) ?? null;
};
var prependImports = (code, imports) => {
  if (!code.startsWith("#!")) {
    return `${imports}${code}`;
  }
...
var buildInlineStaticGetRouteIndex = (routes) => {
  const byAbsoluteUrl = new Map;
  const byRelativeRequestKey = new Map;
  const byResolvedVirtualId = new Map;
  const ambiguousAbsoluteUrls = new Set;
  const ambiguousRelativeKeys = new Set;
  for (const route of routes) {
    ...
    const indexedRoute = {
      ...
      resolvedVirtualId: `${RESOLVED_VIRTUAL_STATIC_GET_PREFIX}${buildVirtualId(route).slice(VIRTUAL_STATIC_GET_PREFIX.length)}`,
      route,
...
var loadInlineStaticGetVirtualModule = (routeIndex, resolvedId) => {
...
var resolveInlineStaticGetVirtualId = (routeIndex, source) => {
...
var transformInlineStaticGetSource = ({
  code,
  routeIndex
}) => {
  ...
  walkAst(ast, (node) => {
    ...
    importCounter += 1;
    const replacement = initArgument ? `(${code.slice(initArgument.start, initArgument.end)}, Promise.resolve(${importName}()))` : `Promise.resolve(${importName}())`;
    replacements.push({
      end: node.end,
  ...
  const importBlock = replacements.map((replacement) => `import { createStaticReplayResponse as ${replacement.importName} } from ${JSON.stringify(replacement.importId)};`).join(`
`);
  return {
...
var DEFAULT_STORAGE_SEED = {
  cookies: [],
  localStorage: {},
  sessionStorage: {}
};
var DEFAULT_SECURITY_HEADERS = {
  detected: {
    coep: null,
    coop: null,
    cspMain: null,
...
var NORMALIZED_HASHED_JSON_FILES = new Set(["runtime-probe.json", "_external/.captured-hosts.json"]);
var ROUTE_BODY_PATH = Symbol("shibukReplayRouteBodyPath");
var ROUTE_BODY_PAYLOAD = Symbol("shibukReplayRouteBodyPayload");
var ROUTE_BODY_TEXT = Symbol("shibukReplayRouteBodyText");
var getBunRuntime = () => {
...
var readWholeFileText = async (filePath) => {
  const bunRuntime = getBunRuntime();
  if (bunRuntime) {
...
var readWholeFileBytes = async (filePath) => {
  const bunRuntime = getBunRuntime();
  if (bunRuntime) {
...
var readJsonFile = async (filePath, fallback) => {
  ...
  try {
    return JSON.parse(await readWholeFileText(filePath));
  } catch {
    return fallback;
  }
};
var readTextFile = async (filePath) => {
...
var normalizeHashedJsonRecord = (relativePath, parsed) => {
...
var hashDirectory = async (rootDir) => {
  const files = (await walkFiles(rootDir)).sort((left, right) => left.localeCompare(right));
  const fileHashes = {};
  const rootHasher = createHash2("sha256");
  for (const filePath of files) {
    const relativePath = path.relative(rootDir, filePath).replace(/\\/g, "/");
    const sha256 = NORMALIZED_HASHED_JSON_FILES.has(relativePath) ? await readJsonFile(filePath, null).then(async (parsed) => {
      if (!parsed) {
        return createHash2("sha256").update(await readWholeFileBytes(filePath)).digest("hex");
      }
      const normalized = normalizeHashedJsonRecord(relativePath, parsed);
      return createHash2("sha256").update(`${JSON.stringify(normalized, null, 2)}
`).digest("hex");
    }) : createHash2("sha256").update(await readWholeFileBytes(filePath)).digest("hex");
...
var assertWorkspaceAslIntegrity = async (captureDir) => {
  const workspaceRoot = path.resolve(captureDir, "..", "..");
  const integrity = await readJsonFile(path.join(workspaceRoot, "asl", ".integrity.json"), null);
  if (!integrity?.files || typeof integrity.rootHash !== "string") {
    return;
  }
  const usesCombinedIntegrity = Object.keys(integrity.files).some((relativePath) => relativePath.startsWith("donor/") || relativePath.startsWith("_network/"));
  ...
  const actualRootHash = usesCombinedIntegrity ? (() => {
    ...
    return networkHash.then((resolvedNetworkHash) => {
      const combinedFiles = Object.entries({
        ...Object.fromEntries(Object.entries(donorHash.fileHashes).map(([relativePath, sha256]) => [
          path.posix.join("donor", relativePath),
          sha256
        ])),
        ...Object.fromEntries(Object.entries(resolvedNetworkHash.fileHashes).map(([relativePath, sha256]) => [
...
`);
  ...
  if (await actualRootHash !== integrity.rootHash) {
    throw new Error(`Replay capture refused because asl/ drifted from asl/.integrity.json: expected ${integrity.rootHash}, got ${await actualRootHash}`);
  }
};
var walkJsonFiles = async (rootDir) => {
  try {
    const entries = await readdir(rootDir, { withFileTypes: true });
    const files = [];
    for (const entry of entries) {
...
var resolveRouteBodyPath = (requestMockRoot, record) => {
...
var loadStoredRequestRoutes = async (requestMockRoot) => {
...
var loadStoredRequestRoute = async (filePath, requestMockRoot) => {
  ...
  if (bodyPath) {
    route[ROUTE_BODY_PATH] = bodyPath;
  }
  return route;
};
var buildStoredRequestRoute = (record) => {
  const lookupKeys = (record.lookupKeys ?? [record.lookupKey]).filter((value) => Boolean(value));
...
var loadSimpleJsonRoutes = async (requestMockRoot) => {
  const routes = [];
...
var loadWebSocketScripts = async (websocketRoot) => {
  const scripts = [];
...
var readReplayRouteBodyText = async (route) => {
...
var loadReplayCapture = async (captureDir) => {
  ...
  const [
    ...
    readJsonFile(path.join(captureDir, "graph", "content-types.json"), {}),
    readJsonFile(path.join(captureDir, "security-headers.json"), DEFAULT_SECURITY_HEADERS),
    readJsonFile(path.join(captureDir, "service-workers.json"), { workers: [] }).then((parsed) => parsed.workers ?? []),
    readJsonFile(path.join(captureDir, "storage", "boot.json"), DEFAULT_STORAGE_SEED),
    readJsonFile(path.join(captureDir, "strip-decisions.json"), {
      decisions: []
    }),
...
var isShapeStubDecision = (decision) => {
...
// src/http-dev-server.ts
var sha1Hex = (value) => {
  return createHash3("sha1").update(value).digest("hex").slice(0, 12);
};
...
var writeRouteResponse = async (res, route) => {
...
var writeResponseObject = async (res, response) => {
...
var maybeServeStripDecision = async ({
  loudTelemetry,
  reqUrl,
  res,
  route,
  ...
  const responseRoute = route && stripDecision.action === "shape-stub-from-capture" && typeof route.body !== "string" ? (await materializeReplayHttpRoutes([route]))[0] ?? route : route;
...
var maybeServeReplayRoute = async ({
  loudTelemetry,
  next,
  res,
  route
...
var createReplayHttpMiddleware = ({
  capture,
  loudTelemetry,
...
var handleReplayRequest = async ({
  ...
  try {
    ...
    if (await maybeServeStripDecision({
      loudTelemetry,
      reqUrl: url,
      res,
      route,
    ...
    if (await maybeServeReplayRoute({
      loudTelemetry,
      next,
      res,
      route
...
// src/precompressed-public-assets.ts
import path2 from "node:path";
import { brotliDecompressSync } from "node:zlib";
var PRECOMPRESSED_CONTENT_TYPES = {
...
var RAW_WASM_MAGIC = new Uint8Array([0, 97, 115, 109]);
var RAW_UNITY_DATA_PREFIX = new TextEncoder().encode("UnityWebData1.0\x00");
var BROTLI_REQUIRED_RESPONSE = `Requested asset requires Brotli support.
`;
var resolvePublicAssetPath = (publicDir, pathname) => {
  const decodedPath = decodeURIComponent(pathname);
  const assetPath = path2.resolve(publicDir, `.${decodedPath}`);
  const publicRoot = `${path2.resolve(publicDir)}${path2.sep}`;
  if (assetPath !== path2.resolve(publicDir) && !assetPath.startsWith(publicRoot)) {
    return null;
  }
  return assetPath;
};
var startsWithBytes = (input, prefix) => {
...
var shouldSendBrotliEncoding = (sourceExtension, body) => {
...
var resolvePrecompressedAssetHeaders = (pathname, body) => {
  if (!pathname.endsWith(".br")) {
    return null;
  }
  const sourcePathname = pathname.slice(0, -".br".length);
  const sourceExtension = path2.extname(sourcePathname).toLowerCase();
  return {
...
var clientAcceptsBrotli = (req) => {
  const acceptEncoding = req.headers["accept-encoding"];
  const headerValue = Array.isArray(acceptEncoding) ? acceptEncoding.join(",") : acceptEncoding ?? "";
  return headerValue.split(",").map((value) => value.trim().toLowerCase()).some((value) => value === "br" || value.startsWith("br;"));
};
var writeBrotliNotAcceptableResponse = ({
  method,
  res
}) => {
  res.statusCode = 406;
...
var servePrecompressedPublicAsset = async ({
  publicDir,
  req,
  res
}) => {
...
var replayLogFile = (workspaceRoot) => path3.join(workspaceRoot, ".lab", "shibuk", "replay-events.jsonl");
var serializeEvent = ({
  count,
  emitterTool,
  emitterVersion,
  ...
  const record = {
    ...
    message: event.message,
    phase: "replay",
    schemaVersion: "shibuk-replay-log/v1",
    source,
...
var ensureWorkspaceInitialized = async (workspaceRoot) => {
  const labManifestPath = path3.join(workspaceRoot, ".lab", "lab.json");
  try {
...
var createReplayRuntimeEventSink = async ({
  ...
  await mkdir(path3.dirname(logPath), { recursive: true });
  ...
  return {
    close: async () => {
      closed = true;
      const results = await Promise.allSettled(Array.from(pendingEvents.keys()).map(async (key) => flushKey(key)));
...
// src/runtime-module.ts
import path4 from "node:path";
import { fileURLToPath } from "node:url";
var packageRoot = path4.resolve(path4.dirname(fileURLToPath(import.meta.url)), "..");
var bundledBrowserRuntimePath = path4.join(packageRoot, "dist", "browser-runtime.js");
var sourceBrowserRuntimePath = path4.join(packageRoot, "src", "browser-runtime.ts");
var bundledBrowserRuntimeSourcePromise = null;
var readBundledBrowserRuntimeFile = async (filePath) => {
...
var loadBundledBrowserRuntimeSource = async () => {
...
var buildReplayRuntimeModule = async ({
  buildBehaviour,
  capture,
...
`;
  ...
  const runtimeCapture = includeDevOnlyFeatures ? {
    ...capture,
    httpRoutes: []
  } : {
    ...capture,
...
// src/websocket-dev-server.ts
import { WebSocketServer } from "ws";
var WEBSOCKET_REPLAY_ENDPOINT = "/__shibuk/replay/websocket";
var decodeBase64 = (value) => Buffer.from(value, "base64");
var coerceBuffer = (value) => {
  return Buffer.isBuffer(value) ? value : Buffer.from(new Uint8Array(value));
};
var rawDataToBuffer = (data2) => {
  if (Array.isArray(data2)) {
    return Buffer.concat(data2.map(coerceBuffer));
  }
  return coerceBuffer(data2);
};
var scheduleServerMessage = (client, message, timers, loudTelemetry, shapeStub, url) => {
  ...
  const timer = setTimeout(() => {
    ...
    const payload = shapeStub && message.binary !== true ? markShapeStubPayload(message.data) : message.binary ? decodeBase64(message.data) : message.data;
    client.send(payload, { binary: message.binary === true });
  }, delay);
  timers.add(timer);
};
var shouldLogServedReplayWebSocketFrame = ({
  loudTelemetry,
  shapeStub
}) => loudTelemetry && shapeStub;
var scheduleServerClose = (client, closeInfo, timers) => {
  if (!closeInfo) {
    return;
  }
...
var normalizeClientPayload = (data2, isBinary) => {
  if (isBinary) {
    const value = rawDataToBuffer(data2);
    return {
      binary: true,
  ...
  return {
    binary: false,
    data: typeof data2 === "string" ? data2 : rawDataToBuffer(data2).toString("utf8")
  };
};
var findMatchingResponseIndex = (responses, payload, responseCursor) => {
...
var attachReplayWebSocketServer = ({
  httpServer,
  loudTelemetry,
  stripDecisions,
  scripts
  ...
  const shapeStubTargets = new Set(resolveActiveStripDecisions(stripDecisions).filter((decision) => isShapeStubDecision(decision) && decision.target.kind === "ws-url").map((decision) => decision.target.value));
  ...
  webSocketServer.on("connection", (client, _request, script) => {
    ...
    client.on("message", (data2, isBinary) => {
      const payload = normalizeClientPayload(data2, Boolean(isBinary));
...
// src/index.ts
var DEFAULT_CAPTURE_PATH = ".lab/capture";
var replayPluginVersionPromise = null;
var loadReplayPluginVersion = async () => {
  replayPluginVersionPromise ??= (async () => {
    try {
      const packageJsonUrl = new URL("../package.json", import.meta.url);
      if (typeof Bun !== "undefined") {
        const parsed2 = await Bun.file(packageJsonUrl).json();
        return typeof parsed2.version === "string" ? parsed2.version : "0.0.0";
      }
      const fsPromises = await import("node:fs/promises");
      const parsed = JSON.parse(await fsPromises.readFile(packageJsonUrl, "utf8"));
...
var shibukReplayPlugin = (options = {}) => {
  const buildBehaviour = options.buildBehaviour ?? "inline-static-get";
  let capture = null;
  let buildRouteIndex = buildInlineStaticGetRouteIndex([]);
  let configRoot = process.cwd();
  let buildOutDir = path5.resolve(configRoot, "dist");
  let command = "serve";
  ...
  const resolveCaptureDir = () => {
    return path5.resolve(configRoot, options.labCapturePath ?? DEFAULT_CAPTURE_PATH);
  };
  ...
  const createPrecompressedPublicAssetMiddleware = (publicDir) => {
    return async (req, res, next) => {
  ...
  const createServiceWorkerMiddleware = (loudTelemetry) => {
    const activeCapture = capture;
    if (!activeCapture) {
      return (_req, _res, next) => next();
    }
    const toWorkerRequestKey = (value) => {
      const workerUrl = new URL(value);
      return `${workerUrl.pathname}${workerUrl.search}`;
    };
    const normalizeScopeHeader = (value) => {
      return new URL(value, "http://localhost").pathname;
    };
    const replayableWorkers = new Map(resolveActiveStripDecisions(activeCapture.stripDecisions.decisions).filter((decision) => decision.category === "service-worker" && decision.action === "replay-script").map((decision) => {
      const worker = activeCapture.serviceWorkers.find((candidate) => candidate.scriptUrl === decision.target.value) ?? null;
      if (!worker?.capturedScript) {
        return null;
      }
      return [toWorkerRequestKey(worker.scriptUrl), worker];
  ...
  const configureReplayMiddlewares = ({
    ...
    middlewares.use(createPrecompressedPublicAssetMiddleware(path5.join(configRoot, "public")));
    middlewares.use(createReplayHttpMiddleware({
      capture,
      loudTelemetry: resolveLoudTelemetry(),
      runtimeEventSink
    }));
...
`;
    ...
    if (typeof Bun !== "undefined") {
      await Bun.write(path5.join(buildOutDir, "_headers"), headersFile);
      await Bun.write(path5.join(buildOutDir, "headers.json"), `${JSON.stringify(headersJson, null, 2)}
`);
      await Bun.write(path5.join(buildOutDir, "SHIPPING.md"), shippingDoc);
      return;
    }
    await fsPromises.writeFile(path5.join(buildOutDir, "_headers"), headersFile);
    await fsPromises.writeFile(path5.join(buildOutDir, "headers.json"), `${JSON.stringify(headersJson, null, 2)}
`);
    await fsPromises.writeFile(path5.join(buildOutDir, "SHIPPING.md"), shippingDoc);
  };
  return {
    async buildEnd() {
    ...
    async configResolved(config) {
      ...
      buildOutDir = path5.resolve(config.root, config.build.outDir);
...
var src_default = shibukReplayPlugin;
export {
  shibukReplayPlugin,
